...
const TalkList: React.FC<TalksListDocument> = ({
  ...
  const personalizableTalks: PersonalizableListItem[] = talks.map((talk) => {
    return {
      ...talk,
      intentTag: JSON.parse((talk.unfrmOptIntentTag as any) || '{}') as IntentTags,
      type: talk._type,
    };
  });
...
const TalkListItem: React.FC<TalkDocument & { buttonText: string }> = (props) => {
  const { title, description, unfrmOptIntentTag } = props;

  let intents = [];
  if (unfrmOptIntentTag) {
    intents = Object.keys(JSON.parse((unfrmOptIntentTag as any) || '{}').intents);
  }
